        ...
        <div class="modal fade show d-block" tabindex="-1" role="dialog" aria-modal="true">
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    @if ($activeModal === 'view' && $viewingUser)
                        <div class="modal-header"><h5 class="modal-title">Szczegóły użytkownika</h5><button type="button" class="btn-close" wire:click="closeModals"></button></div>
                        <div class="modal-body"><dl class="row mb-0"><dt class="col-sm-5">Imię</dt><dd class="col-sm-7">{{ $viewingUser->name }}</dd><dt class="col-sm-5">Email</dt><dd class="col-sm-7">{{ $viewingUser->email }}</dd><dt class="col-sm-5">Nazwa X</dt><dd class="col-sm-7">{{ $viewingUser->x_username ?? 'Nie ustawiono' }}</dd><dt class="col-sm-5">Rola</dt><dd class="col-sm-7">{{ ucfirst($viewingUser->role) }}</dd><dt class="col-sm-5">Ostatnie logowanie</dt><dd class="col-sm-7">{{ $viewingUser->last_login_at?->format('d.m.Y H:i') ?? 'Brak' }}</dd><dt class="col-sm-5">Premium</dt><dd class="col-sm-7">@if ($viewingUser->hasActivePremium())<span class="badge text-bg-warning">Aktywne do {{ $viewingUser->premium_until?->format('d.m.Y') ?? 'bezterminowo' }}</span>@else<span class="text-muted-custom">Nieaktywne</span>@endif</dd></dl>@if ($viewingUser->temporary_password)<div class="temporary-password-panel mt-4"><strong>Hasło tymczasowe</strong><code>{{ $viewingUser->temporary_password }}</code><p class="small mb-0 mt-2">Widoczne do pierwszego logowania użytkownika.</p></div>@endif</div>
                        <div class="modal-footer">@if (in_array(auth()->user()->role, ['superadmin', 'admin'], true))<button class="btn btn-outline-warning" wire:click="resetPassword({{ $viewingUser->id }})">Ustaw nowe hasło tymczasowe</button>@endif<button class="btn btn-primary" wire:click="closeModals">Zamknij</button></div>
                    @elseif ($activeModal === 'create')
                        <div class="modal-header"><h5 class="modal-title">Dodaj użytkownika</h5><button type="button" class="btn-close" wire:click="closeModals"></button></div><form wire:submit="createUser"><div class="modal-body"><div class="row g-3"><div class="col-md-6"><label class="form-label">Imię i nazwisko</label><input class="form-control" wire:model="name" required></div><div class="col-md-6"><label class="form-label">Email</label><input class="form-control" type="email" wire:model="email" required></div><div class="col-md-6"><label class="form-label">Nazwa konta X</label><input class="form-control" wire:model="xUsername" required></div><div class="col-md-6"><label class="form-label">Rola</label><select class="form-select" wire:model="newRole"><option value="kibol">Kibol</option><option value="moderator">Moderator</option>@if (auth()->user()->role === 'superadmin')<option value="admin">Admin</option><option value="superadmin">Superadmin</option>@endif</select></div></div></div><div class="modal-footer"><button type="button" class="btn btn-outline-secondary" wire:click="closeModals">Anuluj</button><button class="btn btn-primary">Utwórz konto</button></div></form>
                    @elseif ($activeModal === 'edit')
                        <div class="modal-header"><h5 class="modal-title">Edytuj użytkownika</h5><button type="button" class="btn-close" wire:click="closeModals"></button></div><form wire:submit="saveUser"><div class="modal-body"><div class="row g-3"><div class="col-md-6"><label class="form-label">Imię i nazwisko</label><input class="form-control" wire:model="name" required></div><div class="col-md-6"><label class="form-label">Email</label><input class="form-control" type="email" wire:model="email" required></div><div class="col-md-6"><label class="form-label">Nazwa konta X</label><input class="form-control" wire:model="xUsername" required></div><div class="col-md-6"><label class="form-label">Rola</label><select class="form-select" wire:model="newRole"><option value="kibol">Kibol</option><option value="moderator">Moderator</option>@if (auth()->user()->role === 'superadmin')<option value="admin">Admin</option><option value="superadmin">Superadmin</option>@endif</select></div></div></div><div class="modal-footer"><button type="button" class="btn btn-outline-secondary" wire:click="closeModals">Anuluj</button><button class="btn btn-primary">Zapisz zmiany</button></div></form>
                    @else
                        <div class="modal-header"><h5 class="modal-title">Nadaj Premium</h5><button type="button" class="btn-close" wire:click="closeModals"></button></div><div class="modal-body"><p class="text-muted-custom">Premium można nadać tylko użytkownikowi, który przynajmniej raz się zalogował.</p><label class="form-label">Okres dostępu</label><select class="form-select" wire:model="premiumDays"><option value="7">7 dni</option><option value="30">30 dni</option></select></div><div class="modal-footer"><button class="btn btn-outline-secondary" wire:click="closeModals">Anuluj</button>@if ($premiumUserId)<button class="btn btn-warning" wire:click="grantPremium({{ $premiumUserId }})">Nadaj Premium</button>@endif</div>
                    @endif
                </div>
            </div>
        </div>
